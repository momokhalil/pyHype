...
def _get_average_value_at_corner(
    Q1: np.ndarray,
    Q2: np.ndarray,
    Q: np.ndarray,
    x1: np.ndarray,
    ...
    _avg = (
        ...
        + Q[stencil[6], :] * (x[stencil[6]] - xc)
    ) / 7

    return _avg


def _get_spatial_terms_at_corner(
    x1: np.ndarray,
    x2: np.ndarray,
    x: np.ndarray,
    y1: np.ndarray,
    ...
    xy = (
        (x1[stencil[0]] - xc) * (y1[stencil[0]] - yc)
...
def least_squares_9_point(
    ...
    # South West
    xc, yc = x[0, 0], y[0, 0]
    ux, uy, x2, y2, xy = _corner_cell(
        Q, QW, QS, Wx, Wy, Sx, Sy, x, y, xc, yc, stencilSW
    )
    den = y2 * x2 - xy**2
    ...
    xc, yc = x[-1, 0], y[-1, 0]
    ux, uy, x2, y2, xy = _corner_cell(
        Q, QW, QN, Wx, Wy, Nx, Ny, x, y, xc, yc, stencilNW
    )
    den = y2 * x2 - xy**2
    ...
    xc, yc = x[0, -1], y[0, -1]
    ux, uy, x2, y2, xy = _corner_cell(
        Q, QE, QS, Ex, Ey, Sx, Sy, x, y, xc, yc, stencilSE
    )
    den = y2 * x2 - xy**2
    ...
    xc, yc = x[-1, -1], y[-1, -1]
    ux, uy, x2, y2, xy = _corner_cell(
        Q, QN, QE, Nx, Ny, Ex, Ey, x, y, xc, yc, stencilNE
    )
    den = y2 * x2 - xy**2
    ...
    for i in range(1, nx - 1):
        ...
        xy = (x[0, i - 1] - xc) * (y[0, i - 1] - yc) + (x[0, i + 1] - xc) * (
            y[0, i + 1] - yc
        )
    ...
    for i in range(1, nx - 1):
        ...
        ux = Q[-1, i - 1, :] * (x[-1, i - 1] - xc) + Q[-1, i + 1, :] * (
            x[-1, i + 1] - xc
        )
        uy = Q[-1, i - 1, :] * (y[-1, i - 1] - yc) + Q[-1, i + 1, :] * (
            y[-1, i + 1] - yc
        )

        xx = (x[-1, i - 1] - xc) ** 2 + (x[-1, i + 1] - xc) ** 2
        yy = (y[-1, i - 1] - yc) ** 2 + (y[-1, i + 1] - yc) ** 2
        xy = (x[-1, i - 1] - xc) * (y[-1, i - 1] - yc) + (x[-1, i + 1] - xc) * (
            y[-1, i + 1] - yc
        )
    ...
    for i in range(1, ny - 1):
        ...
        xy = (x[i - 1, 0] - xc) * (y[i - 1, 0] - yc) + (x[i + 1, 0] - xc) * (
            y[i + 1, 0] - yc
        )
    ...
    for i in range(1, ny - 1):
        ...
        ux = Q[i - 1, -1, :] * (x[i - 1, -1] - xc) + Q[i + 1, -1, :] * (
            x[i + 1, -1] - xc
        )
        uy = Q[i - 1, -1, :] * (y[i - 1, -1] - yc) + Q[i + 1, -1, :] * (
            y[i + 1, -1] - yc
        )

        xx = (x[i - 1, -1] - xc) ** 2 + (x[i + 1, -1] - xc) ** 2
        yy = (y[i - 1, -1] - yc) ** 2 + (y[i + 1, -1] - yc) ** 2
        xy = (x[i - 1, -1] - xc) * (y[i - 1, -1] - yc) + (x[i + 1, -1] - xc) * (
            y[i + 1, -1] - yc
        )
    ...
    for i in range(1, ny - 1):
        for k in range(1, nx - 1):
            ...
            ux = Q[i, k - 1, :] * (x[i, k - 1] - xc) + Q[i, k + 1, :] * (
                x[i, k + 1] - xc
            )
            uy = Q[i, k - 1, :] * (y[i, k - 1] - yc) + Q[i, k + 1, :] * (
                y[i, k + 1] - yc
            )

            xx = (x[i, k - 1] - xc) ** 2 + (x[i, k + 1] - xc) ** 2
            yy = (y[i, k - 1] - yc) ** 2 + (y[i, k + 1] - yc) ** 2
            xy = (x[i, k - 1] - xc) * (y[i, k - 1] - yc) + (x[i, k + 1] - xc) * (
                y[i, k + 1] - yc
            )

            for j in range(k - 1, k + 2):

                ux += Q[i - 1, j, :] * (x[i - 1, j] - xc) + Q[i + 1, j, :] * (
                    x[i + 1, j] - xc
                )
                uy += Q[i - 1, j, :] * (y[i - 1, j] - yc) + Q[i + 1, j, :] * (
                    y[i + 1, j] - yc
                )
